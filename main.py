...
def main():
    ...
    download_from_gmail_mail_from_alertes(gmail, originDate)
    download_from_gmail_mail_from_app(gmail, originDate)

    
def download_from_gmail_mail_from_alertes(gmail, originDate):
...
def download_from_gmail_mail_from_app(gmail, originDate):
    subjects = ['Maison']
    messages = None
    for subject in subjects:
        filter = GmailFilter()
        filter.fromEmail('fredgont').subject(subject).fromDate(originDate)
        if messages is None:
            messages = gmail.listMessages(filter)
        else:
            messages.extend(gmail.listMessages(filter))        
    ...
    for msg in messages:
        ...
        print("msg id [%s], recu [%s] : %s" % (msg['id'], date_received, real_message['snippet']))
        try:
            gmail.saveMessageToFolder(folder = '%s/results_from_app' % dirname(__file__), msg = real_message, overwrite=False)
            print("OK !!!!!! message %s saved" % msg['id'])
        except FileExistsError as e:
            print('already exists')
        except Exception as ee:
        ...
        try:
            gmail.saveMessagePayloadToFolder(folder = '%s/results_from_app' % dirname(__file__), msg = real_message, overwrite=False)
            print("OK !!!!!!! message %s payload saved" % msg['id'])
        except FileExistsError as e:
            print("already exists")
        except Exception as ee:
